...
The **Scheduler** component in Viwo is responsible for managing and executing delayed tasks. It allows scripts to schedule verbs to be executed on entities at a future time.
...
4. The verb is executed using the `evaluate` function from `@viwo/scripting`.
