...
createLibraryTester(ObjectLib, "Object Library", (test) => {
  ...
  test("obj.keys", async () => {
    expect(await evaluate(ObjectLib["obj.keys"]({ a: 1, b: 2 }), ctx)).toEqual(["a", "b"]);
    expect(await evaluate(ObjectLib["obj.keys"]({}), ctx)).toEqual([]);
  });

  test("obj.values", async () => {
    expect(await evaluate(ObjectLib["obj.values"]({ a: 1, b: 2 }), ctx)).toEqual([1, 2]);
  });

  test("obj.entries", async () => {
    expect(await evaluate(ObjectLib["obj.entries"]({ a: 1 }), ctx)).toEqual([["a", 1]]);
  });
  ...
  test("obj.has", async () => {
    ...
    expect(await evaluate(ObjectLib["obj.has"]({ a: 1 }, "b"), ctx)).toBe(false);
  });

  test("obj.del", async () => {
    const localCtx = { ...ctx, locals: {} };
    await evaluate(Core["let"]("o", { a: 1, b: 2 }), localCtx);
    expect(await evaluate(ObjectLib["obj.del"](Core["var"]("o"), "a"), localCtx)).toBe(true);
    expect(await evaluate(Core["var"]("o"), localCtx)).toEqual({ b: 2 });
    expect(await evaluate(ObjectLib["obj.del"](Core["var"]("o"), "c"), localCtx)).toBe(false);
  });

  test("obj.merge", async () => {
    expect(await evaluate(ObjectLib["obj.merge"]({ a: 1 }, { b: 2, a: 3 }), ctx)).toEqual({
      a: 3,
      b: 2,
    });
  });
  ...
  test("obj.map", async () => {
    ...
    expect(await evaluate(ObjectLib["obj.map"]({ a: 1, b: 2 }, inc), ctx)).toEqual({
      a: 2,
      b: 3,
    });
  });
  ...
  test("obj.filter", async () => {
    // (lambda (val key) (> val 1))
    const gt1 = Core["lambda"](["val", "key"], BooleanLib[">"](Core["var"]("val"), 1));
    expect(await evaluate(ObjectLib["obj.filter"]({ a: 1, b: 2 }, gt1), ctx)).toEqual({
      b: 2,
    });
  });
  ...
  test("obj.reduce", async () => {
    ...
    expect(await evaluate(ObjectLib["obj.reduce"]({ a: 1, b: 2 }, sum, 0), ctx)).toBe(3);
  });
  ...
  test("obj.flatMap", async () => {
    ...
    expect(await evaluate(ObjectLib["obj.flatMap"]({ a: 1 }, expand), ctx)).toEqual({
      a: 1,
      a_dup: 1,
    });
  });
