...
describe("List Library", () => {
  ...
  test("list.len", async () => {
    expect(await evaluate(["list.len", ["list.new", 1, 2, 3]], ctx)).toBe(3);
    expect(await evaluate(["list.len", ["list.new"]], ctx)).toBe(0);
  });

  test("list.get", async () => {
    expect(await evaluate(["list.get", ["list.new", 10, 20], 1], ctx)).toBe(20);
    expect(await evaluate(["list.get", ["list.new", 10, 20], 5], ctx)).toBe(
      undefined,
    );
  });
  ...
  test("list.set", async () => {
    ...
    await evaluate(["let", "l", ["list.new", 1, 2, 3]], localCtx);
  ...
  test("list.push/pop", async () => {
    const localCtx = { ...ctx, locals: {} };
    await evaluate(["let", "l", ["list.new", 1, 2]], localCtx);
  ...
  test("list.unshift/shift", async () => {
    const localCtx = { ...ctx, locals: {} };
    await evaluate(["let", "l", ["list.new", 2, 3]], localCtx);
  ...
  test("list.slice", async () => {
    ...
    expect(
      await evaluate(["list.slice", ["list.new", ...list], 1, 3], ctx),
    ).toEqual([2, 3]);
  });

  test("list.splice", async () => {
    const localCtx = { ...ctx, locals: {} };
    await evaluate(["let", "l", ["list.new", 1, 2, 3, 4]], localCtx);
  ...
  test("list.concat", async () => {
    expect(
      await evaluate(["list.concat", ["list.new", 1], ["list.new", 2]], ctx),
    ).toEqual([1, 2]);
  });

  test("list.includes", async () => {
    expect(await evaluate(["list.includes", ["list.new", 1, 2], 2], ctx)).toBe(
      true,
    );
    expect(await evaluate(["list.includes", ["list.new", 1, 2], 3], ctx)).toBe(
      false,
    );
  });
  ...
  test("list.sort", async () => {
    const localCtx = { ...ctx, locals: {} };
    await evaluate(["let", "l", ["list.new", "b", "a", "c"]], localCtx);
  ...
  test("list.map", async () => {
    ...
    expect(
      await evaluate(["list.map", ["list.new", 1, 2, 3], inc], ctx),
    ).toEqual([2, 3, 4]);
  });
  ...
  test("list.filter", async () => {
    ...
    expect(
      await evaluate(["list.filter", ["list.new", 1, 2, 3], gt1], ctx),
    ).toEqual([2, 3]);
  });
  ...
  test("list.reduce", async () => {
    ...
    expect(
      await evaluate(["list.reduce", ["list.new", 1, 2, 3], sum, 0], ctx),
    ).toBe(6);
  });
  ...
  test("list.flatMap", async () => {
    ...
    const dup = [
      "lambda",
      ["x"],
      ["list.new", ["var", "x"], ["+", ["var", "x"], 1]],
    ];
    expect(
      await evaluate(["list.flatMap", ["list.new", 1, 3], dup], ctx),
    ).toEqual([1, 2, 3, 4]);
  });
});
