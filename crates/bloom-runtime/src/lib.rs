...
use bloom_core::{EntityId, WorldStorage};
...
pub struct BloomRuntime {
    ...
    scheduler: Arc<bloom_core::Scheduler>,
    plugins: Arc<Mutex<plugin_loader::PluginRegistry>>,
}

impl BloomRuntime {
    /// Create a new runtime from a database path.
    pub fn open(db_path: &str) -> Result<Self, bloom_core::StorageError> {
    ...
    pub fn open_with_interval(
        db_path: &str,
        interval_ms: u64,
    ) -> Result<Self, bloom_core::StorageError> {
        ...
        let scheduler = Arc::new(bloom_core::Scheduler::new(
    ...
    #[deprecated(note = "Use BloomRuntime::open() instead")]
    pub fn new(storage: WorldStorage) -> Self {
        ...
        let storage = Arc::new(Mutex::new(storage));
        let scheduler_storage = Arc::new(TokioMutex::new(scheduler_storage));
        let scheduler = Arc::new(bloom_core::Scheduler::new(scheduler_storage.clone(), 100));
        let plugins = Arc::new(Mutex::new(plugin_loader::PluginRegistry::new()));
    ...
    pub fn scheduler(&self) -> &Arc<bloom_core::Scheduler> {
        &self.scheduler
    }
...
pub enum ExecutionError {
    ...
    Storage(#[from] bloom_core::StorageError),

    #[error("lua error: {0}")]
    Lua(#[from] mlua::Error),

    #[error("runtime error: {0}")]
    Runtime(#[from] bloom_runtime_luajit::ExecutionError),

    #[error("compile error: {0}")]
    Compile(#[from] bloom_runtime_luajit::CompileError),
