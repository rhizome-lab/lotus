...
pub fn compile_memory(
    ...
    let result = match op {
        "memory.add" => {
            if args.len() != 7 {
                return Err(CompileError::InvalidArguments(format!(
                    "memory.add expects 7 arguments (db_capability, ai_capability, db_path, provider, model, content, metadata), got {}",
                    args.len()
                )));
            }
        ...
        "memory.search" => {
            if args.len() != 7 {
                return Err(CompileError::InvalidArguments(format!(
                    "memory.search expects 7 arguments (db_capability, ai_capability, db_path, provider, model, query, options), got {}",
                    args.len()
                )));
            }
            let db_cap = compile_value(&args[0], false)?;
