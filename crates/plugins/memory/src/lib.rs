//! Memory plugin for Viwo - RAG with content storage and vector search.
//!
...
unsafe fn ensure_tables(
    ...
    // Call __viwo_sqlite_execute to create table if not exists
    lua_getglobal(L, b"__viwo_sqlite_execute\0".as_ptr() as *const c_char);
    if lua_isnil(L, -1) != 0 {
        lua_pop(L, 1);
        return Err("memory: __viwo_sqlite_execute global not found".to_string());
    }
    ...
    // Call __viwo_sqlite_execute(cap, path, sql, params)
...
unsafe extern "C" fn memory_add_lua(L: *mut mlua::ffi::lua_State) -> c_int {
    ...
    if lua_isnil(L, -1) != 0 {
        lua_pop(L, 1);
        return lua_push_error(L, "memory.add: __viwo_sqlite_execute global not found");
    }
    ...
    lua_getglobal(L, b"__viwo_sqlite_query\0".as_ptr() as *const c_char);
    if lua_isnil(L, -1) != 0 {
        lua_pop(L, 1);
        return lua_push_error(L, "memory.add: __viwo_sqlite_query global not found");
    }
